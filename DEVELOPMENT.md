...
| 阶段 | 内容 | 状态 |
|---|---|---|
| 第一阶段：项目基础 | Maven 工程、多环境配置、统一响应/异常/校验、初始化 SQL | ✅ 完成 |
| 第二阶段：认证与权限 | 登录、JWT、用户信息、退出、RBAC 关联、Security 权限拦截 | ✅ 完成 |
| 第三阶段：前端接口联调 | 用户/角色/菜单/个人中心/仪表盘接口，字段与前端类型对齐 | ✅ 完成 |
| 第四阶段：质量与部署 | 48 个接口测试、操作日志、Knife4j 文档、Docker Compose | ✅ 完成 |

补充说明：

- 验收标准已全部达成，逐条核对见 `README.md`。
- 运行方式、环境变量、演示账号见 `README.md`。
- 已知限制（仪表盘演示指标、内存态锁定/黑名单、密码全局失效方案）见 `README.md`「已知限制与生产建议」。
